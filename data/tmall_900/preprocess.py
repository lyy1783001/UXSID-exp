...
def to_df(file_name):
    ...
    df = pd.read_csv(file_name, header=0, names=['uid', 'iid', 'cid', 'sel_id', 'bra_id', 'time', 'btag'])
    df = df[df['btag'] == 0]
    return df
...
def remap(df, max_len):
    ...
    bra_key = sorted(df['bra_id'].unique().tolist())
    bra_len = len(bra_key)
    bra_map = dict(zip(bra_key, range(1, bra_len + 1)))
    df['bra_id'] = df['bra_id'].map(lambda x: bra_map[x])
    ...
    print(f"The number of brands:{bra_len}")
    # print(f"The number of btags:{btag_len}")
    # 423862, 1080666, 1492, 4995, 8036
...
if __name__ == "__main__":
    max_len, min_len = 900, 3
    # 0.5 neg 0.5 pos
    data_path = "../tmall/user_log_format1.csv"
    df = to_df(data_path)
